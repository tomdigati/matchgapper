...
// ═══════════════════════════════════════════════════════════════
// SUPABASE PLAYERS DATA LAYER
// ═══════════════════════════════════════════════════════════════
function dbPlayerToApp(p) {
  return {
...
function appPlayerToDb(p, club, userId) {
  return {
...
async function loadPlayersFromDb(club) {
...
async function savePlayerToDb(player, club, userId) {
...
async function bulkInsertPlayers(players, club, userId) {
...
async function bulkUpsertPlayers(players, club, userId, options = {}) {
  // Load existing players for this club to match against
  ...
  for (const p of players) {
    const row = appPlayerToDb(p, club, userId);
    delete row.id;
    // Match by GHIN first (coerce both to string), then by name
    const match = p.ghin
  ...
  // Run updates — only write fields that were present in the uploaded file
  ...
  // Run inserts — skip if updateOnly mode (sparse file with no status column)
...
// ═══════════════════════════════════════════════════════════════
// SUPABASE SCHEDULE & ASSIGNMENTS DATA LAYER
// ═══════════════════════════════════════════════════════════════
async function loadScheduleFromDb(club) {
...
async function saveScheduleWeekToDb(club, week, weekData, lockData, userId) {
  const row = {
...
async function loadAssignmentsFromDb(club) {
...
async function saveAssignmentsWeekToDb(club, week, weekAssignments, userId) {
  const row = {
    club, week: parseInt(week),
...
// ═══════════════════════════════════════════════════════════════
// CLUB REGISTRATION SCREEN (PGA Pro @pga.com only)
// ═══════════════════════════════════════════════════════════════
...
// ═══════════════════════════════════════════════════════════════
// INVITE SIGNUP SCREEN (accessed via ?invite=TOKEN)
// ═══════════════════════════════════════════════════════════════
...
// ═══════════════════════════════════════════════════════════════
// LOGIN SCREEN (Sign In only — signup requires invitation)
// ═══════════════════════════════════════════════════════════════
// ═══════════════════════════════════════════════════════════════
// PLATFORM ADMIN SIGNUP (bypass invite for god mode emails)
// ═══════════════════════════════════════════════════════════════
...
function Tooltip({ player, position, threshold }) {
  ...
  // Portal to body so it always renders above all stacking contexts
...
function TeamBuilder({ players, schedule, lockState, setLockState, userRole, clubName, assignments, setAssignments }) {
  ...
  // Ensure assignments have entries for all teams when teamCount changes
  useEffect(() => {
    setAssignments(prev => {
  ...
  const handleDrop = (teamIdx, side, pairIdx, playerId) => {
    ...
    setAssignments(prev => {
      ...
      if (w[teamIdx]) {
        ...
        // Strip nulls/stale ids before checking capacity
...
function Dashboard({ players, schedule, lockState, clubName, assignments }) {
  ...
  // Count distinct players who are "maybe" for availability on any week — matches Weekly Coverage
        ...
        <div key={wa.week} style={{ background: "#fff", borderRadius: 12, border: "1px solid #d1d9e6", overflow: "hidden", marginBottom: 16 }}>
          ...
          <div className="mg-week-teams" style={{ display: "grid", gridTemplateColumns: `repeat(${wa.teamCount}, 1fr)`, gap: 0 }}>
            {wa.teams.map((teamSched, idx) => {
              // Get actually assigned players from assignments state
...
function Roster({ players, setPlayers, addPlayerWithSync, clubName, userId, schedule }) {
  ...
  const handleFileUpload = async (e) => {
    ...
    try {
      ...
      // Build set of DB field names that were actually present in the uploaded file
      // Only these fields will be written on UPDATE — everything else is preserved as-is
      ...
      // Upsert to Supabase (update existing by GHIN/name, insert new)
      if (clubName && userId) {
...
function AdminPanel({ currentUserId, clubName }) {
  ...
  const [inviteTab, setInviteTab] = useState("users"); // "users" or "invites"
...
// ═══════════════════════════════════════════════════════════════
// PLATFORM ADMIN GOD MODE — full cross-club management
// ═══════════════════════════════════════════════════════════════
...
function PlatformAdmin({ currentUserId, onEnterClub }) {
  ...
  const [activeSection, setActiveSection] = useState("dashboard"); // dashboard, clubs, users, invites
  const [newInviteEmail, setNewInviteEmail] = useState("");
  ...
  const totalPlayers = approved.length; // placeholder — we'd need a count query
        ...
        <>
          ...
          {activeSection === "dashboard" && (
            <div>
              ...
              {/* Pending Registrations (always visible on overview) */}
              {pending.length > 0 && (
...
// ═══════════════════════════════════════════════════════════════
// ROOT COMPONENT (with Supabase Auth)
// ═══════════════════════════════════════════════════════════════
const TAB_PATHS = {
